...
`;
  ...
  const buildDeterministicResponse = () => {
    ...
    if (qLower.includes("tema") || qLower.includes("motorway") || qLower.includes("road")) {
      responseText = `The Tema Motorway Interchange Upgrade Phase 2 (OCID: ocds-demo-2026-000001) is currently in the Implementation phase with an awarded contract to Accra BuilderCo Ltd. The initial awarded amount was GHS 47,200,000.00, which has been amended to GHS 51,800,000.00 (+9.75% / +GHS 4,600,000.00) due to unforeseen ground conditions requiring sub-soil remediation. Two review signals have been flagged: a Single Bidder review signal during tendering, and a Significant Amendment signal exceeding the 5% threshold. Two interim milestone payments totaling GHS 21,400,000.00 have been processed and confirmed.`;
